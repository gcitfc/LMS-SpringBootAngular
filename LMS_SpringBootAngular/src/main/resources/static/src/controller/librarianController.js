lmsApp.controller("librarianController", function($scope, $http, $window) {
	...
	$scope.updateBranch = function(){
		...
		console.log(branch)
		$http.post("http://localhost:8070/lms/lib/updateBranch", JSON.stringify(branch)).success(function(data){
	...
	$scope.addCopies = function() {
		...
		$http.get("http://localhost:8070/lms/lib/readBookCopies", 
				{ params: {
					    	branchId : branchId
					}
	...
	$scope.updateCopies = function() {
		...
		console.log(copy)
		$http.post("http://localhost:8070/lms/lib/updateCopies", copy).success(function(data){})
	};
})